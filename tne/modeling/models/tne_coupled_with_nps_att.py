...
@Model.register("tne_coupled_model_with_nps_att")
class TNECoupledModel_att(Model):
    """
    Details
    ...
    def __init__(
        ...
        self._prepositions = prepositions #list of strings with the prepositions
        self._freeze = freeze # boolean whether to freeze the MLM

        self._text_field_embedder = text_field_embedder #SpanBERT
        self._context_layer = context_layer
        self._att_layer = att_layer
        self._anchor_feedforward = TimeDistributed(anchor_feedforward) #anchor vector
        self._complement_feedforward = TimeDistributed(complement_feedforward) #complement vector

        self._preposition_scorer = TimeDistributed(preposition_predictor) #prediction
    ...
    def forward(
        ...
        #apply attention between NPs only
        #att_span_embeddings = self.get_span_embeddings_att(text,spans)
        mask = torch.ones((span_embeddings.size()[0],span_embeddings.size()[1]), dtype=torch.bool, device=spans.device)
        att_span_embeddings = self._att_layer(span_embeddings,mask)
        #concatenate both embeddings
        span_embeddings = torch.cat((span_embeddings,att_span_embeddings),dim=2)


        #at the end of this part span embeddings holds encoding of size (2*encoding_dim) for all NP's

        anchor_reps = self._anchor_feedforward(span_embeddings) #anchor representations
        complement_reps = self._complement_feedforward(span_embeddings) #complement representations
        ...
        preposition_scores = self._preposition_scorer(mat.unsqueeze(0)).squeeze(0) #run anchor+complement through network to get scores
    ...
    def get_span_embeddings(self, text, spans):
        ...
        text_embeddings = self._lexical_dropout(self._text_field_embedder(text)) #embedded text with SpanBERT

        # Shape: (batch_size, document_length)
        text_mask = util.get_text_field_mask(text) #mask - not each text the same size
    ...
    def get_span_embeddings_att(self, text, spans):
